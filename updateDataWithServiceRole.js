...
async function updateData() {
  try {
    ...
    if (billsError) {
      console.error('❌ Error clearing bills:', billsError);
    } else {
    ...
    // Step 2: Add new tenant data
    console.log('\n📋 Step 2: Adding new tenant data...');
    ...
    for (let i = 0; i < processedTenants.length; i += batchSize) {
      const batch = processedTenants.slice(i, i + batchSize);
      ...
      if (i + batchSize < processedTenants.length) {
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }
    
    console.log('\n🎉 Data update completed successfully!');
    console.log(`📊 Total tenants added: ${successCount} out of ${newTenants.length}`);
